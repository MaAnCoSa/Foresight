...
def __main__(verbose = 0):
    ...
    #for cr in [0.125, 0.25, 0.5] + [i+1 for i in range(1, 30)]:
    amount_of_monsters = len(monsters_data[str(cr)])

    #for i in range(amount_of_monsters):
        #for j in range(10):
    if monsters_data[str(cr)][0]["name"] not in monster_exceptions:
        party = Party([PC(pc_lvl), PC(pc_lvl), PC(pc_lvl), PC(pc_lvl)])
        ...
        monster_data1 = monsters_data[str(cr)][0]
